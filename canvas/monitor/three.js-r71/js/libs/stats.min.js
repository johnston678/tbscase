    ...
    return {
        REVISION: 11, domElement: f, setMode: t, begin: function () {
            ...
            b > m + 1E3 && (h = Math.round(1E3 * r / (b - m)), p = Math.min(p, h), q = Math.max(q, h), i.textContent = h + " FPS (" + p + "-" + q + ")", a = Math.min(30, 30 - 30 * (h / 100)), c.appendChild(c.firstChild).style.height =
                a + "px", m = b, r = 0);
            if(typeof(canvas_camera_orthographic_url) != "undefined"
                || typeof(canvas_geometry_hierarchy_url) != "undefined"
                || typeof(webgl_custom_attributes_particles2_url) != "undefined"
                || typeof(webgl_shader2_url) != "undefined"
                || typeof(webgl_materials_url) != "undefined")
            {
                addValue(h);
            }
            return b
